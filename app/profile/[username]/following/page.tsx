...
export default function FollowingPage({
  ...
  const loadFollowing = async () => {
    ...
    try {
      ...
      // Check if following list is hidden
      ...
      const followingProfiles = (followsData || []).map((f: any) => f.profiles).filter(Boolean) as Profile[]
      ...
      followingProfiles.forEach((followedUser) => {
        followingMapData[followedUser.id] = followingIds.has(followedUser.id)
      })
        ...
        <main className="min-h-screen pt-14 pb-14 md:pb-6 md:pl-64 md:pt-0 flex justify-center bg-background">
          <div className="flex-1 max-w-[680px] border-x border-border">
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center">
              ...
              <p className="text-muted-foreground">@{username} has chosen to hide their following list.</p>
            </div>
          </div>
        </main>
        <MobileBottomNav />
    ...
    <>
      ...
      <main className="min-h-screen pt-14 pb-14 md:pb-6 md:pl-64 md:pt-0 flex justify-center bg-background">
        <div className="flex-1 max-w-[680px] border-x border-border">
          ...
          <div>
            {following.length === 0 ? (
              <div className="p-12 text-center text-sm text-muted-foreground">Not following anyone yet</div>
            ) : (
              following.map((followedUser) => (
                <UserListItem
                  ...
                  isFollowing={followingMap[followedUser.id] || false}
