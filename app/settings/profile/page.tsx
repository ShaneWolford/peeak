...
import { Loader2, MapPin, Globe, Calendar, Check, ArrowLeft } from "lucide-react"
...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    const fields = [bio, location, website, birthDate, avatarUrl, bannerUrl]
    const filledFields = fields.filter((f) => Boolean(f && f.trim())).length
    setProfileCompletion(Math.round((filledFields / fields.length) * 100))
  }, [bio, location, website, birthDate, avatarUrl, bannerUrl])

  useEffect(() => {
    if (!initialValuesRef.current.bio) return // Wait for initial load
  ...
  }, [bio, location, website, birthDate])
  ...
  }, [bio, location, website, birthDate, hasUnsavedChanges, userId])
            ...
            <CardContent>
              <form onSubmit={handleManualSave} className="space-y-8">
                ...
                <div className="flex justify-end gap-3 pt-4">
                  <Button type="button" variant="outline" onClick={() => router.push("/settings")} disabled={isSaving}>
                    Cancel
                  </Button>
